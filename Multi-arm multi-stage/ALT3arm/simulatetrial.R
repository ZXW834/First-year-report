...
simulate.trial <- function(ii,response.probs, ns, max.ar=0.75, 
  ...
  TypeIerrorgrid=seq(0.98,0.998,0.0001)
  ...
  for(group in 1:nrow(stats)){
    ...
    if(rand.type == "Block"){
      ...
      randomsample=sample(K,n.new,replace = T,prob = c(1-rpblocktwoarm,rpblocktwoarm))
    }
    #Update dataset
...
#---------------------Thall's approach---------------------    
    ...
    #------------------------Allocation bounds restriction (K arm)---------------
    lower=ifelse(alloc.prob.best<(1-max.ar),1-max.ar,alloc.prob.best)
    
    rest=1-sum(lower[lower==(1-max.ar)])
    normalise=rest*lower[-which(lower==(1-max.ar))]/sum(lower[-which(lower==(1-max.ar))])
    
    lower[-which(lower==(1-max.ar))]=normalise
    # upper=ifelse(lower>max.ar,max.ar,lower)
    
    randomprob = lower
    rpblocktwoarm=randomprob[2]
    #Power reduced
    }
#-------------------------------------------------------------   

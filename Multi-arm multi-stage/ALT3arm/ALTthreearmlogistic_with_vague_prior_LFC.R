...
for (i in 1:3){
  Alt.ScenarioLFC1[[i]] = foreach(icount(10000)) %dopar% funsimt(response.probs = response.probs, 
...
for (i in 1:3){
  Alt.ScenarioLFC2[[i]] = foreach(icount(10000)) %dopar% funsimt(response.probs = response.probs, 
...
save(Alt.ScenarioLFC1,Alt.ScenarioLFC2,file = "LFClogisticvagueThall.RData")
